...
class TestCreatorDirect:

    def test_should_direct_list_creator(self):
        setup = _creator_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-creator", "live" if setup["live"] else "unit")
        if _skip:
...
def _creator_direct_setup(mockres):
