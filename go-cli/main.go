// Command Civitai-cli is a CLI + REPL for the Civitai SDK,
...
const prompt = "civitai"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "creator image model model_version tag"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CIVITAI_APIKEY (api key), CIVITAI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CIVITAI_APIKEY carries the API key and
	// CIVITAI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CIVITAI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CIVITAI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCivitaiSDK(opts)
