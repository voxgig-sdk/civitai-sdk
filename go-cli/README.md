...
boru-driven command-line client **and** interactive REPL for the Civitai
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/civitai-cli)
make build
...
./civitai-cli list image

# 5. Override the API base URL for a single call
CIVITAI_BASE=https://api.example.com ./civitai-cli list creator

# 6. No arguments -> interactive REPL
./civitai-cli
civitai> list creator
civitai> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/civitai-cli
   ```
...
Run with no arguments to open a REPL (prompt `civitai>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `creator`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/civitai-cli`. |
...
(`./civitai-cli <expr>`) and interactively (the REPL), and expressions compose the
