...
import (
	...
	omni "github.com/voxgig-sdk/civitai-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CivitaiError); is {
				out := map[string]any{
					"name":    "CivitaiError",
...
func omniCtx(arg any, client *sdk.CivitaiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CivitaiSDK); is {
			client = live
