...
import (
	...
	vs "github.com/voxgig-sdk/civitai-sdk/go/utility/struct"
)

type ModelVersionEntity struct {
	name    string
	client  *core.CivitaiSDK
...
func NewModelVersionEntity(client *core.CivitaiSDK, entopts map[string]any) *ModelVersionEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *ModelVersionEntity) GetName() string { return e.name }
...
func (e *ModelVersionEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *ModelVersionEntity) Deleted() bool {
	return e.deleted
}


func (e *ModelVersionEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewModelVersionEntity(e.client, opts)
}

func (e *ModelVersionEntity) Data(args ...any) any {
...
func (e *ModelVersionEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an ModelVersion; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *ModelVersionEntity) DataTyped(data ...ModelVersion) ModelVersion {
	if len(data) > 0 {
		return typedFrom[ModelVersion](e.Data(asMap(data[0])))
	}
	return typedFrom[ModelVersion](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through ModelVersion (all fields
// optional at the wire level).
func (e *ModelVersionEntity) MatchTyped(match ...ModelVersion) ModelVersion {
	if len(match) > 0 {
		return typedFrom[ModelVersion](e.Match(asMap(match[0])))
	}
	return typedFrom[ModelVersion](e.Match())
}
...
func (e *ModelVersionEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *ModelVersionEntity) Load(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// ModelVersionLoadMatch and returns an ModelVersion. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *ModelVersionEntity) LoadTyped(reqmatch ModelVersionLoadMatch, ctrl map[string]any) (ModelVersion, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return ModelVersion{}, err
	}
	return typedFrom[ModelVersion](res), nil
}



func (e *ModelVersionEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *ModelVersionEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *ModelVersionEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *ModelVersionEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *ModelVersionEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
