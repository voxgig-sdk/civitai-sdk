...
class CivitaiMakeError
{
    public static function call(?CivitaiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CivitaiContext([], null);
        }
        $op = $ctx->op ?? new CivitaiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CivitaiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CivitaiError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CivitaiError('', $msg, $ctx);
