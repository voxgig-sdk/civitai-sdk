...
The Lua SDK for the Civitai API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Creator(nil):list(nil, nil)
...
local result, err = client:Civitai(nil):load(
  { id = "test01" }, nil
)
