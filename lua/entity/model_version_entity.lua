...
local ModelVersionEntity = {}
ModelVersionEntity.__index = ModelVersionEntity


function ModelVersionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ModelVersionEntity)
  self._name = "model_version"
  self._client = client
...
function ModelVersionEntity:get_name()
  return self._name
end


function ModelVersionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ModelVersionEntity.new(self._client, opts)
end
...
function ModelVersionEntity:mark_deleted()
  self._deleted = true
end


function ModelVersionEntity:deleted()
  return true == self._deleted
end


function ModelVersionEntity:data_set(args)
  if args ~= nil then
...
function ModelVersionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ModelVersionEntity:match_set(args)
  if args ~= nil then
...
function ModelVersionEntity:match_get()
...
function ModelVersionEntity:stream(action, args, callopts)
...
---@param reqmatch ModelVersionLoadMatch
---@param ctrl? table
---@return ModelVersion
---@return string? err
function ModelVersionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ModelVersionEntity:_run_op(ctx, post_done)
  local utility = self._utility
