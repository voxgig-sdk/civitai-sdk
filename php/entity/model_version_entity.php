...
// Civitai SDK ModelVersion entity
...
class ModelVersionEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ModelVersionEntity($this->_client, $opts);
    }

    /**
     * @param ModelVersion|array $args ModelVersion data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CivitaiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ModelVersion|array The current ModelVersion data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ModelVersion fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CivitaiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ModelVersion fields).
     */
     ...
     *
     * @param ModelVersionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ModelVersionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ModelVersion|array The loaded ModelVersion as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CivitaiHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
