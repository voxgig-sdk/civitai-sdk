...
class FtCtrl extends CivitaiControl
{
...
class FtClient
{
    ...
    private CivitaiUtility $utility;

    public function __construct(CivitaiUtility $utility, array $options)
    {
    ...
    public function get_utility(): CivitaiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CivitaiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CivitaiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CivitaiConfig::make_feature($name);
    ...
    private static function buildUrl(CivitaiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CivitaiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CivitaiResponse($fetched);
        }
    ...
    private function fail(CivitaiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CivitaiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
