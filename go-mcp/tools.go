...
type Args struct {
	Entity string         `json:"entity" jsonschema:"creator | image | model | model_version | tag"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CivitaiSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "civitai_list",
		Description: "List records from Civitai. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "civitai_load",
		Description: "Load a single record from Civitai. " +
...
func runOp(client *sdk.CivitaiSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CivitaiSDK, name string) (sdk.CivitaiEntity, error) {
	switch strings.ToLower(name) {
	case "creator":
		return client.Creator(nil), nil
	case "image":
		return client.Image(nil), nil
	case "model":
		return client.Model(nil), nil
	case "model_version":
		return client.ModelVersion(nil), nil
	case "tag":
