...
[MCP](https://modelcontextprotocol.io) server exposing the Civitai SDK as
two agent tools — `civitai_list` and `civitai_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/civitai-mcp)
make build

# 2. Provide credentials via the environment
export CIVITAI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user civitai \
  -- /absolute/path/to/civitai-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./civitai-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// civitai_list: first page of records
{ "entity": "creator" }
{ "entity": "creator", "query": { } }

// civitai_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/civitai-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user civitai \
     -- "$PWD"/dist/*/civitai-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `civitai_list` and `civitai_load` tools now appear
   in new sessions. Ask the agent to *"list creator using civitai"*
   and it calls `civitai_list` with `{"entity":"creator"}`.
...
```sh
export CIVITAI_APIKEY=sk_live_xxx            # API key
export CIVITAI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `civitai_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `civitai_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CIVITAI_APIKEY` | API key sent with every request. |
| `CIVITAI_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

creator | image | model | model_version | tag

### Smoke test via HTTP (raw JSON-RPC)

```sh
./civitai-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"civitai_load","arguments":{"entity":"model","query":{"id":1}}}}'
```
