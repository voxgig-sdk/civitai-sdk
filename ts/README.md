...
`client.Creator()` — each with a small set of operations (`list`, `load`)
...
```ts
const model = await client.Model().load({ id: 1 })
```
...
```ts
const model_version = await client.ModelVersion().load({ id: 1 })
```
