...
//     (\`new CivitaiSDK(...)\` / \`CivitaiSDK.test(...)\` become
//     \`CivitaiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"creator":{"test01":{"id":"test01"}},"image":{"test01":{"id":"test01"}},"model":{"test01":{"id":"test01"}},"model_version":{"test01":{"id":"test01"}},"tag":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CivitaiSDK(...)\` and any
// \`CivitaiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CivitaiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CivitaiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
