...
describe('ModelVersionDirect', async () => {
  ...
  test('direct-load-model_version', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-model_version', setup.live)) return
    if (skipIfMissingIds(t, setup, ["hash01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CIVITAI_TEST_MODEL_VERSION_ENTID']
