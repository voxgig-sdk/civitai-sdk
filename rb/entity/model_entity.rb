...
class ModelEntity
  ...
  def data_set(args)
    if args
      @_data = CivitaiHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = CivitaiHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Model, Hash] the loaded Model; raises CivitaiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CivitaiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ModelListMatch, Hash, nil] match filter (any subset of Model fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Model>, Array] the matching Model items; raises CivitaiError on failure
