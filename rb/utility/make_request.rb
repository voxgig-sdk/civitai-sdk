...
module CivitaiUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = CivitaiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CivitaiResponse.new(fetched)
    else
