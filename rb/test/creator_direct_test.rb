...
class CreatorDirectTest < Minitest::Test
  def test_direct_list_creator
    setup = creator_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-creator", setup[:live] ? "live" : "unit")
    if _should_skip
...
def creator_direct_setup(mockres)
